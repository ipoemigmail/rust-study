...
#[tokio::main]
async fn main() {
    let zk_client = Arc::new(ZooKeeper::connect("localhost:2181", time::Duration::from_secs(1), LoggingWatcher).await.unwrap());
    ...
    let tasks = (0..100000).map(|_| {
        ...
        tokio::spawn(async move { s.get_data("/kakao/commerce/kcdl/v2/jobs/meta/dev-hadoop/local/test.app/config/key1", false).await })
    }).collect::<Vec<_>>();
